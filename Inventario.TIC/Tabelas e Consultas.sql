...
AS
	...
	LEFT OUTER JOIN
	(
		--PEGANDO OS LANÇAMENTOS MANUAIS
		SELECT LINHANUMERO, SUM(VALOR) AS VALOR
		FROM DETALHEFATURA
	...
	--POR CENTRO DE CUSTO
	SELECT A.REFERENCIA, ISNULL(USUARIO.CODCCUSTO, '2.02.03') AS CODCCUSTO, ISNULL(USUARIO.CENTROCUSTO, 'Tecnologia da Informação e Comunicação') AS CENTROCUSTO, SUM(A.VALOR) AS VALOR
	...
	GROUP BY ISNULL(USUARIO.CODCCUSTO, '2.02.03'), ISNULL(USUARIO.CENTROCUSTO, 'Tecnologia da Informação e Comunicação'), A.REFERENCIA
...
--testes de composição da NF da VIVO

DECLARE @REFERENCIA VARCHAR(20) = '01/2020'



SELECT FATURA.LINHANUMERO, SUM(ISNULL(FATURA.VALOR, 0) + ISNULL(SERVICO.VALOR, 0) + ISNULL(MANUAL.VALOR, 0)) AS VALOR
FROM 
(
	-- FATURA
	SELECT LINHANUMERO, SUM(VALOR) AS VALOR
...
/*CONSULTAS DAS FATURAS DA VIVO - 15/04/2020*/

/*NF DA VIVO*/

DROP TABLE #TMPFATURAVIVO
...
SELECT L.NUMERO AS LINHANUMERO, A.REFERENCIA, A.VALOR, ISNULL(USUARIO.NOME, 'TIC') AS NOME, 
ISNULL(USUARIO.CODCCUSTO, '2.02.03') AS CODCCUSTO, ISNULL(USUARIO.CENTROCUSTO, 'Tecnologia da Informação e Comunicação') AS CENTROCUSTO
FROM #TMPFATURAVIVO A
LEFT JOIN LINHA L ON L.NUMERO = A.LINHANUMERO
LEFT OUTER JOIN 
(
...
--/*NF DA CLARO*/
DROP TABLE #TMPFATURACLARO

DECLARE @REFERENCIA VARCHAR(20) = '03/2020'

SELECT LINHANUMERO, SUM(VALOR) AS VALOR, @REFERENCIA AS REFERENCIA
INTO #TMPFATURACLARO
FROM DETALHEFATURA
WHERE SECAOCHAMADA IN ('Para Outros Estados', 'Para Dentro do Estado', 'Ligações de Longa Distância') AND
REFERENCIA = @REFERENCIA
GROUP BY LINHANUMERO
HAVING SUM(VALOR) <> 0
