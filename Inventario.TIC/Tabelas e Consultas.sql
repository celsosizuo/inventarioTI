...
AS
	SELECT C.CHAPA, P.NOME, P.CPF, C.CODSITUACAO  
...
AS
	...
	SELECT C.CHAPA, C.NOME + ' (' + C.CODSITUACAO + ')' AS NOME, ISNULL(C.CPF, '') AS CPF, C.
...
IF NOT EXISTS (SELECT * FROM SYS.objects WHERE TYPE = 'U' AND NAME = 'TERMOCELULAR')
	...
	(
		...
		PRIMARY KEY (ID),
		UNIQUE (LINHAID, APARELHOID, CARREGADORID),
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTCELULAR')
	DROP PROCEDURE POSTCELULAR

GO

CREATE PROCEDURE POSTCELULAR
--  ID			INT IDENTITY,
    @LINHAID		INT,
    @USUARIOID	INT,
    @APARELHOID	INT
AS
    INSERT CELULARES VALUES (
    @LINHAID,
    @USUARIOID,
    @APARELHOID)        
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETECELULAR')
	DROP PROCEDURE DELETECELULAR

GO

CREATE PROCEDURE DELETECELULAR
    @ID			INT
AS 
    DELETE CELULARES WHERE ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETCELULAR')
	DROP PROCEDURE GETCELULAR

GO

CREATE PROCEDURE GETCELULAR
AS
	SELECT *
	FROM CELULARES C
	INNER JOIN LINHA L ON L.ID = C.LINHAID
