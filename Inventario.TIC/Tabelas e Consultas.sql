...
/***************************************************************************************************************************************************************/
											/*********   INÍCIO DO MÓDULO TERMO DE COMPTUADORES / NOTEBOOKS   *********/
/***************************************************************************************************************************************************************/


IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'TERMOCOMPUTADOR')
	CREATE TABLE TERMOCOMPUTADOR
	(
		ID							INT IDENTITY,
		USUARIOID					INT,
		COMPUTADORID				INT,
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTTERMOCOMPUTADOR')
	DROP PROCEDURE POSTTERMOCOMPUTADOR

GO

CREATE PROCEDURE POSTTERMOCOMPUTADOR
	@USUARIOID					INT,
	@COMPUTADORID				INT,
	@DISPOSITIVOALUGADOID		INT,
	@CARREGADORID				INT,
	...
	@LINKDEVOLUCAO				VARCHAR(MAX)
AS
	INSERT TERMOCOMPUTADOR VALUES (
	@USUARIOID,
	@COMPUTADORID,
	@DISPOSITIVOALUGADOID,
	@CARREGADORID,
	@GESTORID,
	@DATAENTREGA,
	@VALORDISPOSITIVO,
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTTERMOCOMPUTADOR')
	DROP PROCEDURE PUTTERMOCOMPUTADOR

GO

CREATE PROCEDURE PUTTERMOCOMPUTADOR
	@USUARIOID					INT,
	@COMPUTADORID				INT,
	@DISPOSITIVOALUGADOID		INT,
	@CARREGADORID				INT,
	...
	@ID							INT
AS
	UPDATE TERMOCOMPUTADOR SET
	USUARIOID = @USUARIOID,
	COMPUTADORID = @COMPUTADORID,
	DISPOSITIVOALUGADOID = @DISPOSITIVOALUGADOID,
	CARREGADORID = @CARREGADORID,
	GESTORID = @GESTORID,
	DATAENTREGA = @DATAENTREGA,
	VALORDISPOSITIVO = @VALORDISPOSITIVO,
	DATADEVOLUCAO = @DATADEVOLUCAO,
	MOTIVO = @MOTIVO,
	LINKENTREGA = @LINKENTREGA,
	LINKDEVOLUCAO = @LINKDEVOLUCAO
	WHERE ID = @ID	

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETETERMOCOMPUTADOR')
	DROP PROCEDURE DELETETERMOCOMPUTADOR

GO

CREATE PROCEDURE DELETETERMOCOMPUTADOR
	@ID			INT
AS
	DELETE TERMOCOMPUTADOR
	WHERE ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETTERMOCOMPUTADOR')
	DROP PROCEDURE GETTERMOCOMPUTADOR

GO

CREATE PROCEDURE GETTERMOCOMPUTADOR
AS
	SELECT T.*,
	CASE WHEN T.DATADEVOLUCAO IS NULL AND T.LINKENTREGA IS NULL THEN
		'Ativo Sem Termo Escaneado'
	ELSE CASE WHEN T.DATADEVOLUCAO IS NULL AND T.LINKENTREGA IS NOT NULL THEN
		'Ativo Com Termo Escaneado'
	ELSE CASE WHEN T.DATADEVOLUCAO IS NOT NULL AND T.LINKDEVOLUCAO IS NULL THEN
		'Devolvido Sem Termo Escaneado'
	ELSE CASE WHEN T.DATADEVOLUCAO IS NOT NULL AND T.LINKDEVOLUCAO IS NOT NULL THEN
		'Devolvido Com Termo Escaneado'
	END END END END AS STATUS,
	U.*, C.*, D.*, CA.*, G.*, TA.*
	FROM TERMOCOMPUTADOR T
	LEFT JOIN USUARIO U ON U.ID = T.USUARIOID
	LEFT JOIN COMPUTADORES C ON C.ID = T.COMPUTADORID
	LEFT JOIN DISPOSITIVOALUGADO D ON D.ID = T.DISPOSITIVOALUGADOID
	LEFT JOIN CARREGADOR CA ON CA.ID = T.CARREGADORID
	LEFT JOIN GESTOR G ON G.ID = T.GESTORID
	LEFT JOIN TERMOCOMPUTADORACESSORIO TA ON TA.TERMOCOMPUTADORID = T.ID 

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PATHTERMOCOMPUTADORENTREGA')
	DROP PROCEDURE PATHTERMOCOMPUTADORENTREGA

GO

CREATE PROCEDURE PATHTERMOCOMPUTADORENTREGA
	@TERMOCOMPUTADORID				INT,
	@LINKENTREGA				VARCHAR(MAX)
AS
	UPDATE TERMOCOMPUTADOR
	SET LINKENTREGA = @LINKENTREGA
	WHERE ID = @TERMOCOMPUTADORID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PATHTERMOCOMPUTADORDEVOLUCAO')
	DROP PROCEDURE PATHTERMOCOMPUTADORDEVOLUCAO

GO

CREATE PROCEDURE PATHTERMOCOMPUTADORDEVOLUCAO
	@TERMOCOMPUTADORID				INT,
	@LINKDEVOLUCAO				VARCHAR(MAX)
AS
	UPDATE TERMOCOMPUTADOR
...
IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'TERMOCOMPUTADORACESSORIO')
	CREATE TABLE TERMOCOMPUTADORACESSORIO
	(
		ID							INT IDENTITY,
		TERMOCOMPUTADORID			INT,
		...
		PRIMARY KEY (ID),
		FOREIGN KEY (TERMOCOMPUTADORID) REFERENCES TERMOCOMPUTADOR (ID)
	)

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTTERMOCOMPUTADORACESSORIO')
	DROP PROCEDURE POSTTERMOCOMPUTADORACESSORIO

GO

CREATE PROCEDURE POSTTERMOCOMPUTADORACESSORIO
	@TERMOCOMPUTADORID			INT,
	@NOMEACESSORIO				VARCHAR(100),
	@VALOR						DECIMAL(18,2)
AS
	INSERT TERMOCOMPUTADORACESSORIO VALUES (@TERMOCOMPUTADORID, @NOMEACESSORIO, @VALOR)	

	SELECT SCOPE_IDENTITY()

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTTERMOCOMPUTADORACESSORIO')
	DROP PROCEDURE PUTTERMOCOMPUTADORACESSORIO

GO

CREATE PROCEDURE PUTTERMOCOMPUTADORACESSORIO
	@NOMEACESSORIO				VARCHAR(100),
	@VALOR						DECIMAL(18,2),
	@TERMOCOMPUTADORID			INT,
	@ID							INT
AS
	UPDATE TERMOCOMPUTADORACESSORIO
	SET NOMEACESSORIO = @NOMEACESSORIO,
	VALOR = @VALOR
	WHERE TERMOCOMPUTADORID = @TERMOCOMPUTADORID AND
	ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETETERMOCOMPUTADORACESSORIO')
	DROP PROCEDURE DELETETERMOCOMPUTADORACESSORIO

GO

CREATE PROCEDURE DELETETERMOCOMPUTADORACESSORIO
	@ID							INT
AS
	DELETE TERMOCOMPUTADORACESSORIO
	WHERE ID = @ID	

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETTERMOCOMPUTADORACESSORIO')
	DROP PROCEDURE GETTERMOCOMPUTADORACESSORIO

GO

CREATE PROCEDURE GETTERMOCOMPUTADORACESSORIO
	@TERMOCELULARID				INT
AS
	SELECT *
	FROM TERMOCOMPUTADORACESSORIO
	WHERE @TERMOCELULARID = @TERMOCELULARID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PATHTERMOCOMPUTADORCANCELARDEVOLUCAO')
	DROP PROCEDURE PATHTERMOCOMPUTADORCANCELARDEVOLUCAO

GO

CREATE PROCEDURE PATHTERMOCOMPUTADORCANCELARDEVOLUCAO
	@TERMOCOMPUTADORID			INT
AS
	UPDATE TERMOCOMPUTADOR
	SET DATADEVOLUCAO = NULL,
	MOTIVO = NULL
	WHERE ID = @TERMOCOMPUTADORID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'RELATORIOTERMOENTREGACOMPUTADOR')
	DROP PROCEDURE RELATORIOTERMOENTREGACOMPUTADOR

GO

CREATE PROCEDURE RELATORIOTERMOENTREGACOMPUTADOR
	@TERMOCOMPUTADORID			INT
AS
SELECT T.ID, T.DATAENTREGA, T.VALORDISPOSITIVO AS VALORCOMPUTADOR, T.DATADEVOLUCAO, T.MOTIVO,
U.NOME, U.CHAPA, U.CPF,
ISNULL(C.ATIVONOVO, D.ATIVO) AS ATIVO,
CA.MARCA AS MARCACARREGADOR, CA.NUMSERIE AS NUMSERIECARREGADOR, CA.VALOR AS VALORCARREGADOR,
TA.NOMEACESSORIO, TA.VALOR AS VALORACESSORIO,
G.NOME AS GESTOR
FROM TERMOCOMPUTADOR T
LEFT JOIN USUARIO U ON U.ID = T.USUARIOID
LEFT JOIN COMPUTADORES C ON C.ID = T.COMPUTADORID
...
/************************************************************************************************************************************************/
/* SCRIPT NOVO RATEIO IMPRESSORAS - 07/07/2021 */
/************************************************************************************************************************************************/

IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'TIPOIMPRESSAO')
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTTIPOIMPRESSAO')
	DROP PROCEDURE POSTTIPOIMPRESSAO

GO

CREATE PROCEDURE POSTTIPOIMPRESSAO
	@TIPOIMPRESSAO				VARCHAR(50)
AS
	INSERT TIPOIMPRESSAO VALUES (@TIPOIMPRESSAO)
	SELECT SCOPE_IDENTITY()

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTTIPOIMPRESSAO')
	DROP PROCEDURE PUTTIPOIMPRESSAO

GO

CREATE PROCEDURE PUTTIPOIMPRESSAO
	@TIPOIMPRESSAO				VARCHAR(50),
	@ID							INT
AS
	UPDATE TIPOIMPRESSAO
	SET DESCRICAO = @TIPOIMPRESSAO
	WHERE ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETETIPOIMPRESSAO')
	DROP PROCEDURE DELETETIPOIMPRESSAO

GO

CREATE PROCEDURE DELETETIPOIMPRESSAO
	@ID							INT
AS
	DELETE TIPOIMPRESSAO
	WHERE ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETTIPOIMPRESSAO')
	DROP PROCEDURE GETTIPOIMPRESSAO

GO

CREATE PROCEDURE GETTIPOIMPRESSAO
AS
	SELECT * FROM TIPOIMPRESSAO

GO

-- exec sp_rename 'TIPOIMPRESSAO.[TIPOIMPRESSAO]', 'DESCRICAO', 'column'

-- DBCC CHECKIDENT('TIPOIMPRESSAO', RESEED, 0)

IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'REFERENCIA')
	...
	(
		...
		DATAINICIO				SMALLDATETIME,
		DATAFIM					SMALLDATETIME,
		[STATUS]				VARCHAR(1) -- A-ABERTO F-FECHADO

		PRIMARY KEY (ID)
	)

-- exec sp_rename 'REFERENCIA.[REFERENCIA]', 'REF', 'column'

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTREFERENCIA')
	DROP PROCEDURE POSTREFERENCIA

GO

CREATE PROCEDURE POSTREFERENCIA
	@REF						VARCHAR(20),
	@DATAINICIO					SMALLDATETIME,
	@DATAFIM					SMALLDATETIME,
	@STATUS						VARCHAR(1) 
AS
	INSERT REFERENCIA (REF, DATAINICIO, DATAFIM, [STATUS]) VALUES (@REF, @DATAINICIO, @DATAFIM, @STATUS)

	SELECT SCOPE_IDENTITY()

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTREFERENCIA')
	DROP PROCEDURE PUTREFERENCIA

GO

CREATE PROCEDURE PUTREFERENCIA
	@REF						VARCHAR(20),
	@DATAINICIO					SMALLDATETIME,
	@DATAFIM					SMALLDATETIME,
	@STATUS						VARCHAR(1),
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETEREFERENCIA')
	DROP PROCEDURE DELETEREFERENCIA

GO

CREATE PROCEDURE DELETEREFERENCIA
	@ID							INT
AS
	DELETE REFERENCIA 
	WHERE ID = @ID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETREFERENCIA')
	DROP PROCEDURE GETREFERENCIA

GO

CREATE PROCEDURE GETREFERENCIA
AS
...
CREATE PROCEDURE GETREFERENCIASTATUS
AS
	SELECT 'A' AS CODIGO, 'Aberto' AS DESCRICAO
	UNION ALL
	SELECT 'F' AS CODIGO, 'Fechado' AS DESCRICAO




IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'FORNECEDOR')
	CREATE TABLE FORNECEDOR
	(
		ID						INT IDENTITY,
		NOME					VARCHAR(100),
		CNPJ					VARCHAR(20),
		DATAINICONTRATO			SMALLDATETIME,
		DATAFIMCONTRATO			SMALLDATETIME
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTFORNECEDOR')
	DROP PROCEDURE POSTFORNECEDOR

GO

CREATE PROCEDURE POSTFORNECEDOR
	@NOME					VARCHAR(100),
	@CNPJ					VARCHAR(20),
	@DATAINICONTRATO			SMALLDATETIME,
	@DATAFIMCONTRATO			SMALLDATETIME
AS
	INSERT FORNECEDOR (NOME, CNPJ, DATAINICONTRATO, DATAFIMCONTRATO) VALUES (@NOME, @CNPJ, @DATAINICONTRATO, @DATAFIMCONTRATO)
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTFORNECEDOR')
	DROP PROCEDURE PUTFORNECEDOR

GO

CREATE PROCEDURE PUTFORNECEDOR
	@NOME						VARCHAR(100),
	@CNPJ						VARCHAR(20),
	@DATAINICONTRATO			SMALLDATETIME,
	@DATAFIMCONTRATO			SMALLDATETIME,
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETEFORNECEDOR')
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETFORNECEDOR')
	DROP PROCEDURE GETFORNECEDOR

GO

CREATE PROCEDURE GETFORNECEDOR
AS
...
IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'IMPRESSORAS')
	...
	(
		...
		SETOR					VARCHAR(100),
		CCUSTO					VARCHAR(50),
		VALORFIXO				DECIMAL(18,4),
		OBSERVACAO				VARCHAR(MAX),
		[STATUS]				VARCHAR(1) -- A-ATIVO // I-INATIVO

		PRIMARY KEY (ID),
		FOREIGN KEY (FORNECEDORID) REFERENCES FORNECEDOR (ID),
		FOREIGN KEY (TIPOIMPRESSAOID) REFERENCES TIPOIMPRESSAO (ID)  
	)

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTIMPRESSORAS')
	DROP PROCEDURE POSTIMPRESSORAS

GO

CREATE PROCEDURE POSTIMPRESSORAS
	@FORNECEDORID			INT,
	@TIPOIMPRESSAOID		INT,
	@NUMSERIE				VARCHAR(100),
	@MODELO					VARCHAR(100),
	...
	@STATUS					VARCHAR(1) -- A-ATIVO // I-INATIVO
AS
	INSERT IMPRESSORAS (FORNECEDORID, TIPOIMPRESSAOID, NUMSERIE, MODELO, SETOR, CCUSTO, VALORFIXO, OBSERVACAO, [STATUS])
	VALUES (@FORNECEDORID, @TIPOIMPRESSAOID, @NUMSERIE, @MODELO, @SETOR, @CCUSTO, @VALORFIXO, @OBSERVACAO, @STATUS)

	SELECT SCOPE_IDENTITY()
GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTIMPRESSORAS')
	DROP PROCEDURE PUTIMPRESSORAS

GO

CREATE PROCEDURE PUTIMPRESSORAS
	@FORNECEDORID			INT,
	@TIPOIMPRESSAOID		INT,
	@NUMSERIE				VARCHAR(100),
	@MODELO					VARCHAR(100),
...
AS
	...
	TIPOIMPRESSAOID = @TIPOIMPRESSAOID, 
	NUMSERIE = @NUMSERIE, 
	MODELO = @MODELO, 
	SETOR = @SETOR, 
	CCUSTO = @CCUSTO, 
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETEIMPRESSORAS')
	DROP PROCEDURE DELETEIMPRESSORAS

GO

CREATE PROCEDURE DELETEIMPRESSORAS
	@ID						INT
AS
	DELETE IMPRESSORAS
	WHERE ID = @ID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETIMPRESSORAS')
	DROP PROCEDURE GETIMPRESSORAS

GO

CREATE PROCEDURE GETIMPRESSORAS
AS
	SELECT * 
	FROM IMPRESSORAS I
	LEFT JOIN TIPOIMPRESSAO TI ON TI.ID = I.TIPOIMPRESSAOID
	LEFT JOIN FORNECEDOR F ON F.ID = I.FORNECEDORID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETIMPRESSORASRESPONSE')
	DROP PROCEDURE GETIMPRESSORASRESPONSE

GO

CREATE PROCEDURE GETIMPRESSORASRESPONSE
AS
	SELECT  I.ID, I.MODELO, I.SETOR, I.NUMSERIE, TI.DESCRICAO AS TIPOIMPRESSAO, I.[STATUS], F.NOME AS FORNECEDOR, I.FORNECEDORID, I.TIPOIMPRESSAOID
	FROM IMPRESSORAS I
	LEFT JOIN TIPOIMPRESSAO TI ON TI.ID = I.TIPOIMPRESSAOID
	LEFT JOIN FORNECEDOR F ON F.ID = I.FORNECEDORID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETIMPRESSORASTATUS')
	DROP PROCEDURE GETIMPRESSORASTATUS

GO

CREATE PROCEDURE GETIMPRESSORASTATUS
AS
	SELECT 'A' AS CODIGO, 'Ativa' AS DESCRICAO
	UNION ALL
	SELECT 'I' AS CODIGO, 'Inativa' AS DESCRICAO

GO



IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'CONTADORES')
	...
	(
		...
		CONTADORATUAL			INT,
		DATACONTADORATUAL		SMALLDATETIME,
		PAGINASIMPRESSAS		INT

		PRIMARY KEY (ID),
		FOREIGN KEY (REFERENCIAID) REFERENCES REFERENCIA (ID),
		FOREIGN KEY (TIPOIMPRESSAOID) REFERENCES TIPOIMPRESSAO (ID),
		FOREIGN KEY (IMPRESSORAID) REFERENCES IMPRESSORAS (ID)
	)

GO


IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTCONTADORES')
	DROP PROCEDURE POSTCONTADORES

GO

CREATE PROCEDURE POSTCONTADORES
	@REFERENCIAID			INT,
	@TIPOIMPRESSAOID		INT,
	@IMPRESSORAID			INT,
	@CONTADORANTERIOR		INT,
	...
	@PAGINASIMPRESSAS		INT
AS
	INSERT CONTADORES (REFERENCIAID, TIPOIMPRESSAOID, IMPRESSORAID, CONTADORANTERIOR, CONTADORATUAL, DATACONTADORATUAL, PAGINASIMPRESSAS)
	VALUES (@REFERENCIAID, @TIPOIMPRESSAOID, @IMPRESSORAID, @CONTADORANTERIOR, @CONTADORATUAL, @DATACONTADORATUAL, @PAGINASIMPRESSAS)

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTCONTADORES')
	DROP PROCEDURE PUTCONTADORES

GO

CREATE PROCEDURE PUTCONTADORES
	@REFERENCIAID			INT,
	@TIPOIMPRESSAOID		INT,
	@IMPRESSORAID			INT,
	@CONTADORANTERIOR		INT,
	...
	@ID						INT
AS
	UPDATE CONTADORES SET
	REFERENCIAID = @REFERENCIAID, 
	TIPOIMPRESSAOID = @TIPOIMPRESSAOID, 
	IMPRESSORAID = @IMPRESSORAID, 
	CONTADORANTERIOR = @CONTADORANTERIOR, 
	CONTADORATUAL = @CONTADORATUAL, 
	DATACONTADORATUAL = @DATACONTADORATUAL, 
	PAGINASIMPRESSAS = @PAGINASIMPRESSAS
	WHERE ID = @ID

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETECONTADORES')
	DROP PROCEDURE DELETECONTADORES

GO

CREATE PROCEDURE DELETECONTADORES
	@ID						INT
AS
	DELETE CONTADORES
	WHERE ID = @ID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETCONTADORES')
	DROP PROCEDURE GETCONTADORES

GO

CREATE PROCEDURE GETCONTADORES
AS
...
IF NOT EXISTS (SELECT * fROM SYS.objects WHERE type = 'U' AND name = 'FRANQUIAS')
	...
	(
		...
		TIPOIMPRESSAOID			INT,
		QUANTIDADE				INT,
		VALORPAGINAFRANQUIA		DECIMAL(18,4),
		VALORPAGINAEXCEDENTE	DECIMAL(18,4)
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'POSTFRANQUIAS')
	DROP PROCEDURE POSTFRANQUIAS

GO

CREATE PROCEDURE POSTFRANQUIAS
	@FORNECEDORID				INT,
	@TIPOIMPRESSAOID			INT,
	@QUANTIDADE					INT,
	@VALORPAGINAFRANQUIA		DECIMAL(18,4),
	@VALORPAGINAEXCEDENTE		DECIMAL(18,4)
AS
	INSERT FRANQUIAS (FORNECEDORID, TIPOIMPRESSAOID, QUANTIDADE, VALORPAGINAFRANQUIA, VALORPAGINAEXCEDENTE) 
	VALUES (@FORNECEDORID, @TIPOIMPRESSAOID, @QUANTIDADE, @VALORPAGINAFRANQUIA, @VALORPAGINAEXCEDENTE)

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'PUTFRANQUIAS')
	DROP PROCEDURE PUTFRANQUIAS

GO

CREATE PROCEDURE PUTFRANQUIAS
	@FORNECEDORID				INT,
	@TIPOIMPRESSAOID			INT,
	@QUANTIDADE					INT,
	@VALORPAGINAFRANQUIA		DECIMAL(18,4),
...
AS
	...
	TIPOIMPRESSAOID = @TIPOIMPRESSAOID, 
	QUANTIDADE = @QUANTIDADE, 
	VALORPAGINAFRANQUIA = @VALORPAGINAFRANQUIA, 
	VALORPAGINAEXCEDENTE = @VALORPAGINAEXCEDENTE
	WHERE ID = @ID	

GO

IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'DELETEFRANQUIAS')
	DROP PROCEDURE DELETEFRANQUIAS

GO

CREATE PROCEDURE DELETEFRANQUIAS
	@ID							INT
AS
	DELETE FRANQUIAS
	WHERE ID = @ID
...
IF EXISTS (SELECT * fROM SYS.objects WHERE type = 'P' AND name = 'GETFRANQUIAS')
	DROP PROCEDURE GETFRANQUIAS

GO

CREATE PROCEDURE GETFRANQUIAS
AS
